...
export const useRemoteServersStore = create<RemoteServersState>()(
  persist(
    (set, get) => ({
      ...
      setClientFactory: (factory) => set({ clientFactory: factory }),
      setSocketFactory: (factory) => set({ socketFactory: factory }),
      ...
      openRemoteThread: async (desktopId, threadId) => {
        ...
        try {
          ...
          const socket = get().socketFactory(client.websocketUrl(ticket, snapshot.snapshotSeq));
          activeThreadSocket = socket;
          socket.onmessage = (event) => {
            try {
              const message = client.parseSocketMessage(String(event.data));
              if (message.type === "event") dispatchRemoteSupervisorEvent(message.event);
            } catch {
              // Ignore malformed frames; the next snapshot refresh re-syncs.
            }
          };
          socket.onclose = () => {
            if (activeThreadSocket === socket) activeThreadSocket = null;
          };
        } catch {
          // No live stream (ticket/socket failed); the static history still shows.
        }
      },

      closeRemoteThread: () => {
        if (activeThreadSocket) {
          try {
      ...
      pairServer: async ({ endpoint, token }) => {
        ...
        const record: RemoteServerRecord = {
          ...
          endpoint: environment.endpoints.httpBaseUrl,
      ...
      refreshServer: async (desktopId) => {
        ...
        try {
          const snapshot = await get()
            .clientFactory(server.endpoint, server.accessToken)
            .snapshot();
          setRuntime({ status: "online", projects: snapshot.projects, threads: snapshot.threads });
        } catch (error) {
          setRuntime({
            status: "error",
      ...
      connectAll: async () => {
        ...
        connectAllInFlight = Promise.all(
          get().servers.map((server) => get().refreshServer(server.desktopId)),
        )
          .then(() => undefined)
          .finally(() => {
