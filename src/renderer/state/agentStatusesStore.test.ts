...
describe("persisted agent status cache", () => {
  it("invalidates v26 snapshots so per-provider credentials are re-probed", async () => {
    ...
    expect(options.version).toBe(27);
  ...
  it("invalidates v25 capability snapshots so provider capabilities are re-probed", async () => {
    const options = useAgentStatusesStore.persist.getOptions();
