...
export function usePushLifecycle(input: PushLifecycleInput): void {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [{ ActivityBridge }, { configureLiveActivities }] = await Promise.all([
          import("@poracode/activity-bridge"),
          import("./liveActivityController"),
        ]);
    ...
    return () => {
      ...
      void import("./liveActivityController").then(({ configureLiveActivities }) =>
  ...
  useEffect(() => {
    ...
    return () => {
      void import("./pushRegistration").then(({ teardownPushListeners }) =>
        teardownPushListeners({ resetSentState: true }),
      );
    };
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { syncPushRegistration, teardownPushListeners, unregisterPush } =
        await import("./pushRegistration");
      if (cancelled) return;
