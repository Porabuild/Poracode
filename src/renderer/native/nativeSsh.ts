...
export interface NativeSshConnectResult {
...
export function isNativeSshAuthenticationError(error: unknown): boolean {
  if (
    typeof error === "object" &&
    error !== null &&
  ...
  return error instanceof Error && error.cause !== undefined
    ? isNativeSshAuthenticationError(error.cause)
    : false;
}
...
export async function probeNativeSshHost(
  target: string,
  port = 22,
...
export async function disconnectNativeSsh(connectionId: string): Promise<void> {
  await SshBridge.disconnect({ connectionId });
}

export function resetNativeSshRuntimeForTests(): void {
  manifestPromise = null;
  archivePromise = null;
}
