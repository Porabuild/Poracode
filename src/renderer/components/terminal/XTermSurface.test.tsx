...
describe("XTermSurface", () => {
  ...
  it("refits on the first live output so the launch-race resize reaches the PTY", async () => {
    ...
    act(() => {
      emitEvent({
        ...
        data: "frame",
        outputLength: 5,
        terminalInstanceId: "generation-test",
      });
    });
    await flushFrame();
  ...
  it("refits when the first live output arrives during empty scrollback hydration", async () => {
    vi.spyOn(HTMLElement.prototype, "clientWidth", "get").mockReturnValue(800);
    vi.spyOn(HTMLElement.prototype, "clientHeight", "get").mockReturnValue(600);
    ...
    await flushFrame();
    expect(state.bridge.readTerminalScrollback).toHaveBeenCalledWith({ threadId: "test-1" });
    const callsBeforeOutput = state.bridge.resizeTerminal.mock.calls.length;
    expect(callsBeforeOutput).toBeGreaterThan(0);

    act(() => {
      emitEvent({
        type: "thread-output",
        threadId: "test-1",
        data: "first and only prompt",
        outputLength: 21,
        terminalInstanceId: "generation-test",
      });
      resolveScrollback("");
    ...
    expect(terminal().write).toHaveBeenCalledWith("first and only prompt");
    expect(state.bridge.resizeTerminal.mock.calls.length).toBeGreaterThan(callsBeforeOutput);
    expect(state.bridge.resizeTerminal).toHaveBeenLastCalledWith({
      threadId: "test-1",
      cols: 80,
      rows: 24,
