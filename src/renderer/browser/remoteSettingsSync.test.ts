...
describe("pushDesktopSettingsDiff push ordering", () => {
  ...
  it("forwards worktree placement changes to the selected desktop", async () => {
    applyDesktopSettings({
      ...settings("v0"),
      worktreeStorageMode: "global",
      worktreeBasePath: "D:\\worktrees",
      wslWorktreeBasePath: "/mnt/worktrees",
    } as RemoteSettings);
    const committed = {
      ...settings("v0"),
    ...
    pushDesktopSettingsDiff(client, {
      ...input("v0"),
      worktreeStorageMode: committed.worktreeStorageMode,
      worktreeBasePath: committed.worktreeBasePath,
      wslWorktreeBasePath: committed.wslWorktreeBasePath,
    } as SharedSettingsInput);
    await flush();
  ...
  it("accepts a new desktop hydration while the previous desktop still has a pending push", async () => {
    applyDesktopSettings(settings("desktop-a"));
    const pending = deferred<RemoteSettings>();
    const client = {
      updateSettings: vi.fn<RemoteDesktopClient["updateSettings"]>(() => pending.promise),
    } as unknown as RemoteDesktopClient;

    pushDesktopSettingsDiff(client, input("desktop-a-edit"));
    resetDesktopSettings();
    applyDesktopSettings(settings("desktop-b"));

    expect(h.applyExternalSharedSettings).toHaveBeenLastCalledWith(settings("desktop-b"));
    pending.resolve(settings("late-desktop-a"));
    await flush();

    pushDesktopSettingsDiff(client, input("desktop-b-edit"));
