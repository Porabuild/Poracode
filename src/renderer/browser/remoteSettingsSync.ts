 ...
 * The browser client's settings store holds both kinds of keys: device-local ones that
...
export function pushDesktopSettingsDiff(
  ...
  client
    ...
    .then((next) => {
      ...
      if (generation === settingsGeneration && token === latestPushToken) desktopSettings = next;
    })
    .catch(() => {
      ...
      if (generation === settingsGeneration && token === latestPushToken) desktopSettings = null;
    })
    .finally(() => {
      if (generation === settingsGeneration) pendingPushes -= 1;
    });
}
