...
export function buildCodexArgvFor(
  ...
  const computerUseMcpEnv = buildCodexComputerUseMcpEnv(launchOptions?.computerUseMcp);
  const mcpEnv = { ...(browserMcpEnv ?? {}), ...(computerUseMcpEnv ?? {}) };
...
export function buildCodexAppServerCommand(
  ...
  const computerUseMcpEnv = buildCodexComputerUseMcpEnv(options?.computerUseMcp);
  const mcpEnv = { ...(browserMcpEnv ?? {}), ...(computerUseMcpEnv ?? {}) };
  const args = [
  ...
  if (location.kind === "wsl") {
    ...
    return {
      ...
      args: [
        ...
        ...Object.entries(mcpEnv).map(([name, value]) => `${name}=${value}`),
        wslExecPath ?? "codex",
        ...args,
      ],
    };
  ...
  return buildAgentCommand(
    ...
    args,
    wslExecPath,
    Object.keys(mcpEnv).length > 0 ? mcpEnv : undefined,
  );
}
