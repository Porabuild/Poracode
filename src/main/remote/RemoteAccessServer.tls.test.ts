...
/** Reads the listener's actual leaf-certificate fingerprint over a real TLS
 * handshake — the shape a main-process `certFingerprintProbe` supplies. */
async function probeFingerprint(port: number | string, hostname = "127.0.0.1"): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    const socket = tlsConnect(
      ...
      () => {
        ...
        resolve(certificate.fingerprint256.replaceAll(":", "").toLowerCase());
      },
    );
...
describe("RemoteAccessServer TLS (Gate 6 item 4.2)", () => {
  it("listens HTTPS, carries the certificate fingerprint in the pairing QR, and answers over https", async () => {
    ...
    await expect(probeFingerprint(new URL(info.httpBaseUrl).port)).resolves.toBe(
  ...
  it("pairs a probe-capable client over https and pins the observed certificate", async () => {
    ...
    const client = new RemoteDesktopClient(info.httpBaseUrl, undefined, insecureTlsFetch, {
      certFingerprintProbe: () => probeFingerprint(new URL(info.httpBaseUrl).port),
  ...
  it("refuses every request once a pinned client meets a different certificate", async () => {
    ...
    const info = await server.start();
    const actual = await probeFingerprint(new URL(info.httpBaseUrl).port);
