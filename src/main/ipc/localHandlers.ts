...
export function createLocalIpcHandlers(
  ...
  return defineMainLocalIpcHandlers({
    ...
    getAutomationsSnapshot: (query) => ({
      schedules: options.scheduleService.list(),
      runs: dbListScheduleRunInbox(query),
      unreadCount: dbGetUnreadScheduleRunCount(),
    }),
    ...
    getScheduleRunInbox: (query) => dbListScheduleRunInbox(query),
    updateScheduleRunState: (payload) => dbUpdateScheduleRunState(payload),
    cancelScheduleRun: ({ id }) => options.scheduleService.cancelRun(id),
