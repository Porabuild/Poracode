...
export function emitSwiftPairingMachine(): string {
  ...
  const rules = spec.phaseAfterFailure
    .map((rule) => {
      const from = rule.from.map((phase) => `.${phase}`).join(", ");
      const target = rule.target === null ? "nil" : `.${rule.target}`;
      return `    RemotePairingPhaseRule(from: [${from}], requiresRetainedCredential: ${rule.hasRetainedCredential}, target: ${target}),`;
    })
    .join("\n");
  ...
  const viewerScopes = spec.scopes.presets.viewer
    .map((scope) => `        ${quote(scope)},`)
    .join("\n");
  const separatorEscape = spec.duplicateDetection.fingerprint.separator;
...
${spec.phases.map((phase) => `  case ${phase}`).join("\n")}
}
...
${rules}
  ...
  // MARK: Scope-request guard (${spec.scopes.emptyRequestMeans})
...
${viewerScopes}
  ...
  public static func fingerprint(endpoint: String, credential: String) -> String {
    let material = Data("\\(endpoint)\\u{${separatorEscape.charCodeAt(0).toString(16)}}\\(credential)".utf8)
  ...
  static let maxHostDisplayCharacters: Int = ${spec.deepLink.hostDisplayFallbackMaxCharacters}
