import { portablePascalName } from "./names";
...
const HEADER = [
  ...
  `// src/shared/remote/contract/pairingMachineSpec.ts (spec version ${PAIRING_MACHINE_SPEC.specVersion}).`,
...
function kotlinPhase(phase: string): string {
  return portablePascalName(phase);
}

export function emitKotlinPairingMachine(): string {
  const errors = validatePairingMachineSpec(PAIRING_MACHINE_SPEC);
  if (errors.length > 0) throw new Error(`pairing machine spec is invalid: ${errors.join("; ")}`);
  const spec = PAIRING_MACHINE_SPEC;
  const rules = spec.phaseAfterFailure
    .map((rule) => {
      const from = rule.from.map((phase) => `RemotePairingPhase.${kotlinPhase(phase)}`).join(", ");
      const target =
        rule.target === null ? "null" : `RemotePairingPhase.${kotlinPhase(rule.target)}`;
      return `        RemotePairingPhaseRule(setOf(${from}), ${rule.hasRetainedCredential}, ${target}),`;
    })
    .join("\n");
  const standardScopes = spec.scopes.standardOrder
    .map((scope) => `        ${quote(scope)},`)
    .join("\n");
  const operatorScopes = spec.scopes.presets.operator
    .map((scope) => `        ${quote(scope)},`)
    .join("\n");
  const viewerScopes = spec.scopes.presets.viewer
    .map((scope) => `        ${quote(scope)},`)
    .join("\n");
  const separatorCode = spec.duplicateDetection.fingerprint.separator.charCodeAt(0);
  const sessionRead = quote("session:read");
  const sessionOperate = quote("session:operate");
...
${spec.phases.map((phase) => `    ${kotlinPhase(phase)},`).join("\n")}
}
...
${rules}
    ...
    // ---------------------------------------------------------------
    // Scope-request guard (${spec.scopes.emptyRequestMeans})
    // ---------------------------------------------------------------
...
${viewerScopes}
    ...
    fun canReadScopes(scopes: List<String>): Boolean =
        filterKnownScopes(scopes).contains(${sessionRead})

    fun canOperateScopes(scopes: List<String>): Boolean =
        filterKnownScopes(scopes).contains(${sessionOperate})
     ...
     * (spec: ${spec.duplicateDetection.fingerprint.algorithm} ${spec.duplicateDetection.fingerprint.encoding}). */
    fun fingerprint(endpoint: String, credential: String): String {
        val material = "$endpoint\\u${separatorCode.toString(16).padStart(4, "0")}$credential"
    ...
    private val maxHostDisplayCharacters: Int = ${spec.deepLink.hostDisplayFallbackMaxCharacters}
