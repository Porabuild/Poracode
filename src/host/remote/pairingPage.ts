...
export function buildLocalPairingPageHtml(input: {
  readonly httpBaseUrl: string;
  /** V6 A.3: SHA-256 leaf fingerprint for self-signed LAN TLS. Browsers cannot pin. */
  readonly certFingerprint?: string;
}): string {
  const endpointJson = jsonForScript(input.httpBaseUrl);
  const fingerprintJson = input.certFingerprint
    ? jsonForScript(`sha256:${input.certFingerprint}`)
    : "null";
  const fingerprintHint = input.certFingerprint
    ? `      <p>This host uses a self-signed TLS certificate. Browsers cannot pin it — compare this fingerprint to the value shown on the desktop before you continue. Prefer a relay or Tailscale HTTPS for web clients.</p>
      <p>Certificate fingerprint</p>
      <code class="endpoint" id="fingerprint"></code>`
    : "";
...
${fingerprintHint}
  ...
  <script>
    ...
    var fingerprint = ${fingerprintJson};
    var fingerprintEl = document.getElementById("fingerprint");
    if (fingerprintEl && fingerprint) fingerprintEl.textContent = fingerprint;
  </script>`,
  });
}
