...
async function startForwardWithTicket(
  ...
  return { forward, ticket: gateway.mintConnectTicket(forward.id, "test-session") };
}
...
describe("RemotePortForwardGateway.startForward / stopForward", () => {
  ...
  it("destroys live sockets and closes the listener on dispose", async () => {
    ...
    expect(() => gateway.mintConnectTicket(forward.id, "test-session")).toThrow(RemoteHttpError);
  });
...
describe("RemotePortForwardGateway connect authentication (plan item 4.5)", () => {
  ...
  it("rejects a wrong or foreign-forward ticket", async () => {
    ...
    const ticketA = gateway.mintConnectTicket(forwardA.id, "test-session");
  ...
  it("burns a connect ticket on first use and binds tickets to the minting session", async () => {
    const { port: targetPort } = await startEchoServer();
    const gateway = makeGateway({ bindHost: "127.0.0.1", candidatePorts: [targetPort] });
    const { forward, ticket } = await startForwardWithTicket(gateway, targetPort);

    await gateway.startForward(targetPort);
    const secondTicket = gateway.mintConnectTicket(forward.id, "test-session");
    expect(secondTicket).not.toBe(ticket);

    const echoed = await connectAndExchange(forward.listenPort, "first use", ticket);
    expect(echoed).toBe("first use");
    await expect(
      connectAndWaitForClose(forward.listenPort, `${ticket}\n`),
    ).resolves.toBeUndefined();
    ...
    const otherSessionTicket = gateway.mintConnectTicket(forward.id, "other-session");
    gateway.revokeSessionTickets("other-session");
    await expect(
      connectAndWaitForClose(forward.listenPort, `${otherSessionTicket}\n`),
    ).resolves.toBeUndefined();
  });

  it("refuses to mint a connect ticket for an unknown forward", async () => {
    const gateway = makeGateway({ bindHost: "127.0.0.1" });
    expect(() => gateway.mintConnectTicket("no-such-forward", "test-session")).toThrow(
      RemoteHttpError,
    );
  });
});
