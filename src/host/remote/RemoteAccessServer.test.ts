...
describe("RemoteAccessServer", () => {
  ...
  it("burns a minted image ticket over HTTP when the minting session is revoked", async () => {
    const dir = createTempDir();
    const pngBytes = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x08]);
    const imagePath = join(dir, "pixel.png");
    writeFileSync(imagePath, pngBytes);
  ...
  it("scopes transcript content per connection without breaking approvals or replay", async () => {
    ...
    expect(await replay.next()).toEqual({
      type: "event",
      seq: 2,
      // The V6 event envelope names its delivery space on every frame.
      space: "loopback",
  ...
  it("keys the pairing rate limit per forwarded client behind a loopback relay hop", async () => {
    ...
    // Both requests arrive from loopback (as they would behind the relay) with
    // the hop marker, and carry distinct x-forwarded-for hops, so each gets
    // its own bucket and neither is throttled despite maxAttempts: 1.
    ...
    // A second attempt from device A (same forwarded hop + marker) is throttled.
  ...
  it("ignores spoofed X-Forwarded-For on a loopback socket without the relay hop marker", async () => {
    const server = new RemoteAccessServer({
      truncateThreadRuntime: () => {},
      appVersion: "1.0.0",
      identity: { desktopId: "desktop-test", label: "Test Desktop" },
    ...
    const first = await fetch(tokenUrl, {
      method: "POST",
      headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.10" },
      body,
    });
    const spoofed = await fetch(tokenUrl, {
      method: "POST",
  ...
  it("ignores a client-set hop marker plus rotating X-Forwarded-For on one budget", async () => {
    const server = new RemoteAccessServer({
      truncateThreadRuntime: () => {},
      appVersion: "1.0.0",
      identity: { desktopId: "desktop-test", label: "Test Desktop" },
    ...
    const rotated = await fetch(tokenUrl, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-forwarded-for": "203.0.113.20",
  ...
  it("honors X-Forwarded-For when the socket matches a trusted proxy (exact address and CIDR)", async () => {
    // The test client dials from loopback, so an exact `127.0.0.1` entry and a
    // `127.0.0.0/8` CIDR both match the socket: distinct forwarded hops get
    // distinct buckets (V6 A.6).
    for (const trustedProxies of [["127.0.0.1"], ["127.0.0.0/8"]]) {
      ...
      const otherHop = await fetch(tokenUrl, {
        method: "POST",
        headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.20" },
        body,
      });
      expect(`${trustedProxies.join()} first ${first.status}`).toBe(
        `${trustedProxies.join()} first 401`,
      );
      expect(`${trustedProxies.join()} otherHop ${otherHop.status}`).toBe(
        `${trustedProxies.join()} otherHop 401`,
      );

      // The first forwarded hop's budget is now spent.
      const retry = await fetch(tokenUrl, {
        method: "POST",
        headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.10" },
        body,
      });
      expect(`${trustedProxies.join()} retry ${retry.status}`).toBe(
        `${trustedProxies.join()} retry 429`,
      );
    }
  });

  it("ignores X-Forwarded-For when no configured trusted proxy matches the socket", async () => {
    const server = new RemoteAccessServer({
      truncateThreadRuntime: () => {},
      appVersion: "1.0.0",
      identity: { desktopId: "desktop-test", label: "Test Desktop" },
    ...
    // The loopback socket matches no trusted entry, so rotating forwarded hops
    // share the one socket-address budget.
    const first = await fetch(tokenUrl, {
      method: "POST",
      headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.10" },
      body,
  ...
  it("revokes the session family, closes live sockets, and audits refresh reuse", async () => {
    ...
    const tokenResponse = await fetch(new URL("/oauth/token", info.httpBaseUrl), {
      ...
      body: JSON.stringify({
        grantType: "pairing-token",
        credential,
        client: { label: "Reuse detector", deviceType: "desktop" },
      }),
    });
    expect(tokenResponse.status).toBe(200);
    ...
    const rotated = await fetch(new URL("/oauth/token", info.httpBaseUrl), {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
    ...
    expect(events.map((event) => event.kind)).toContain("refresh_reuse");
    expect(server.listAccessSessions()).toEqual([]);
    ws.close();
  });
  ...
  it("serves port discovery/forwarding via the injected gateway, scope-gated", async () => {
    ...
    const connectTicket = gateway.mintConnectTicket(forwardResult.forward.id, "test-session");
  ...
  it("forwards allowlisted git calls and enforces scope + allowlist", async () => {
    ...
    // (startThread joined the remote allowlist, so this uses a procedure that
    // stays supervisor-only.)
    const blockedResponse = await fetch(new URL("/api/git/call", info.httpBaseUrl), {
      method: "POST",
      headers: fullHeaders,
      body: JSON.stringify({ procedure: "getCrossagentRouting", payload: {} }),
    });
    expect(blockedResponse.status).toBe(403);
    await expect(blockedResponse.json()).resolves.toMatchObject({
      error: { code: "git_procedure_not_allowed" },
    });
    expect(calls.some((c) => c.name === "getCrossagentRouting")).toBe(false);
