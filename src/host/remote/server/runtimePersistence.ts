import { dbApplyThreadRuntimeEvents, dbReplaceThreadRuntimeSnapshot } from "@/host/db";
...
/**
 * Applies canonical supervisor runtime events directly to SQLite — chat items
 * and, for `usage.spent`, the token usage ledger. The same writer is used by
 * desktop main and the headless remote host, so durability never depends on a
 * mounted renderer or a full transcript rewrite.
 */
function persistRuntimeEvent(event: RemoteBroadcastEvent): void {
  switch (event.type) {
    case "thread-runtime-event":
      ...
      recordUsageSpentFromRuntimeEvents(event.threadId, [event.event]);
      return;
    case "thread-runtime-events":
      dbApplyThreadRuntimeEvents(event.threadId, event.events);
      recordUsageSpentFromRuntimeEvents(event.threadId, event.events);
      return;
    case "thread-runtime-events-multi":
      for (const batch of event.batches) {
        dbApplyThreadRuntimeEvents(batch.threadId, batch.events);
        recordUsageSpentFromRuntimeEvents(batch.threadId, batch.events);
      }
      return;
    case "thread-reset":
