...
export const threadTools: ToolDomain = {
  ...
  handlers: {
    ...
    read_thread: async (args, ctx) => {
      const { threadId, limit, before, maxChars } = readArgsSchema.parse(args);
      requireThread(ctx, threadId);
      // Fenced ordered-transcript read: content is exactly the committed prefix
      // at the pinned cursor, or a typed persistence refusal (never a short
      // transcript served as current).
      const page = await dbGetThreadConversationItemsPage(
        threadId,
        before,
