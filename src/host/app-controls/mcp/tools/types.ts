...
export interface AppControlsToolContext {
   ...
   * `false` when no UI is connected; a promise result is consumed by the caller
   * (never awaited into the durable operation) so a delayed or rejected native
   ...
   * never precede; no-op when the thread row no longer exists.
   */
  updateThreadRow(threadId: string, mutate: (thread: Thread) => Thread): void;
  /**
   * Publish a bounded remote-membership/paint invalidation for the affected
   * thread rows, so list clients learn about host-local writes (MCP metadata
   * updates, schedule-created threads) without a full catalog refresh. Optional
   * for legacy embedders.
   */
  publishThreadsChanged?(threadIds: readonly string[]): void;
  /**
   * Diagnostics sink for background failures the caller must consume (e.g. a
   * rejected thread-command mirror). Optional for legacy embedders.
   */
