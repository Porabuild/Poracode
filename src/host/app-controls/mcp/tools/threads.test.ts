...
const { getConversationPage } = vi.hoisted(() => ({
  getConversationPage:
    vi.fn<
      (threadId: string, before: number | undefined, limit: number) => Promise<PersistedRuntimePage>
    >(),
}));
...
describe("read_thread", () => {
  it("returns canonical user prompt content and bounded assistant text", async () => {
    getConversationPage.mockResolvedValue({
      nextCursor: 12,
      items: [
        {
          id: "user-1",
    ...
    await expect(
      threadTools.handlers.read_thread!({ threadId: "source", limit: 2 }, ctx),
    ).resolves.toEqual({
      threadId: "source",
      messageCount: 2,
      nextCursor: 12,
  ...
  it("allows a caller to opt into a larger per-message result", async () => {
    getConversationPage.mockResolvedValue({
      nextCursor: null,
      items: [
        {
    ...
    const result = (await threadTools.handlers.read_thread!(
  ...
  it("returns display text, not streamed text a display hook replaced or suppressed", async () => {
    getConversationPage.mockResolvedValue({
      nextCursor: null,
      items: [
        {
    ...
    const result = (await threadTools.handlers.read_thread!({ threadId: "source" }, ctx)) as {
  ...
  it("keeps assistant image markers when display text is rewritten or suppressed", async () => {
    getConversationPage.mockResolvedValue({
      nextCursor: null,
      items: [
        {
