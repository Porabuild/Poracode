...
describe("Poracode app control tools — threads", () => {
  ...
  it("update_thread persists the DB row and closes the run without a renderer, claiming no UI delivery", async () => {
    const threads = [makeThread({ id: "a", title: "Old", status: "finished" })];
    const { ctx, emitRemoteThreadCommand, supervisor, updateThreadRow, updatedRows } = context({
      threads,
    ...
    // The async mirror answer cannot be observed before returning, so no
    // delivery claim is made either way.
    expect(result.note).toBeUndefined();
    // Commands are still emitted (attempted) after the authoritative commit.
    expect(emitRemoteThreadCommand).toHaveBeenCalled();
    expect(updateThreadRow).toHaveBeenCalledWith("a", expect.any(Function));
    // done is a host-owned lifecycle effect: it does not wait for a renderer.
    expect(supervisor.closeThread).toHaveBeenCalledWith({ threadId: "a" });
