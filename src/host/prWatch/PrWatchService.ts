 ...
 * `@/main` import; the composition wires the canonical launcher.
 */
export interface PrWatchThreadRequest {
  projectId: string;
  prompt: string;
  agentKind: AgentKind;
  model: string;
...
export interface PrWatchServiceOptions {
  ...
  createThread(request: PrWatchThreadRequest): Promise<PrWatchThreadLaunch>;
