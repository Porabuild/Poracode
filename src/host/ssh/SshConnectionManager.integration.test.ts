...
describe.skipIf(!target || !identityFile)("SshConnectionManager real SSH", () => {
  ...
  // C.1: the e2e environment case uses the standalone host composition's
  // SshConnectionManager, not a desktop-only constructor.
  ...
  const manager =
    hostServices.sshConnectionManager ??
