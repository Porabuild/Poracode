...
- **iOS icon color:** default navigation, toolbar, composer, menu-trigger, provider, and ordinary action symbols to neutral primary/secondary styling. Apply a neutral tint to the owning `Button` or `Menu`, not only its nested `Image`, because native toolbar chrome can otherwise inherit the app accent. Reserve colored symbols or small status indicators for a real semantic state such as selection, active work, warning, failure, or connectivity; a provider identity by itself is not a colored state.
- **iOS primary-action reachability:** on phone layouts, keep a page's safe, central action in the bottom thumb-reachable area instead of the top bar or the horizontal center. Put the primary action at the lower trailing side, especially when the lower leading side already carries navigation, search, dismissal, or another established action. Reuse `PoracodeBottomActionDock` for one action or `PoracodeBottomActionBar` for supporting leading actions plus a primary trailing action, and build each with `PoracodeCircleButton` or `PoracodeCircleMenu`; do not recreate a floating circle, blur, padding recipe, or safe-area inset in each page. Keep ordinary action symbols neutral with the secondary foreground/tint, reserving accent color for a semantic selected or prominent state.
- **iOS compact action chrome:** prefer icon-only native circular controls for familiar actions, with localized accessibility labels. Keep search on the lower leading side and add/create on the lower trailing side when both are present. Avoid word buttons, accent-colored ordinary icons, and detached rows that consume vertical space.
- **iOS tabs:** use a real native selection control and its grouped Liquid Glass treatment for mutually exclusive modes. Do not imitate tabs with independent circular buttons arranged in the center. On phone screens, keep mode switching near the bottom action area when it is a frequent main-screen control.
- **iOS information density:** page titles belong in the native navigation header. Remove avoidable top whitespace and oversized gaps; content should begin beneath the header and leave only the clearance required by bottom controls and safe areas.
- **Mobile visual verification:** when comparing PWA and native app simulators side by side, explicitly switch both surfaces to English first and capture the same state in both light and dark appearance when theme behavior is in scope.
- **iOS component reuse:** before styling a control locally, search `Features/Components/` and nearby feature conventions. Build pages from shared Poracode components and extract a focused reusable component when two or more pages need the same placement, surface, or interaction pattern. Keep the page responsible for its action and state; keep shared components responsible for geometry, material, and consistent interaction styling.
...
- **PWA-first visual correction:** when a live installed PWA exposes a shared mobile layout defect, correct and verify the compact PWA surface first, then align native clients to the same behavior in their platform idiom. Skip this ordering only when the defect is demonstrably native-only.
- **Mobile web action chrome:** put the page title in `MobilePageHeader`, remove avoidable top whitespace, and keep frequent actions in one thumb-reachable bottom row. Reuse `MobilePageBottomBar`, `MobilePageBottomAction`, `MobileCircleButton`, and `LightballTabs`: search belongs on the lower leading side, add/create on the lower trailing side, and a real segmented tab control belongs between them at the same control height. Keep ordinary icons neutral; reserve accent color for semantic selection or status.
