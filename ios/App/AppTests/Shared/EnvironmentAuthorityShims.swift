...
// Shared harness mirrors of the app environment parent-authority transport graph.
// Portable Swift packages symlink this file so the fail-closed transport seam
// has one source of truth without importing the full app model graph.
...
struct RemoteClientError: LocalizedError, Sendable, Equatable {
  ...
  init(
    ...
    responseEvidence: [String: String]? = nil
  ) {
