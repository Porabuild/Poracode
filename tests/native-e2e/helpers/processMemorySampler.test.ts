...
vi.mock("./asyncSampling.ts", async (importOriginal) => ({
  ...
  readProcessOutput: async () => probe.tables.shift() ?? null,
}));
...
describe("process memory evidence", () => {
  it("retains an earlier own-process peak separately from the process-tree peak", async () => {
    probe.tables = [
      "10 1 400\n20 10 100\n30 20 50\n40 1 9000\n",
      "10 1 100\n20 10 800\n30 20 50\n40 1 9000\n",
    ];
    ...
    expect(sampler.summary()).toMatchObject({ samplerVersion: 3, samples: 2, probeFailures: 0 });
  });

  it("preserves measured peaks after the root exits or a later probe fails", async () => {
  ...
  it("reports unobserved memory as unknown rather than zero", async () => {
