...
| Real-provider strict journeys        | Pass locally for Claude (via the z.ai-backed profile), Codex, Devin, Qoder and Muse; Gemini has no credentials; the Anthropic-backed Claude path and a provider-qualification runner remain unproven | Gemini credentials or an explicit exclusion; a run with the default Claude profile                                |
