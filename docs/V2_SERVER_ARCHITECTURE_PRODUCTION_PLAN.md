...
Status: active implementation and qualification; refined 2026-09-22 UTC after Crossagents rounds R1–R3 (§0.7). **About 88% of the overall goal is achieved; every remaining gate needs hardware, runner enrollment, a first stable release or an explicit product decision. This is not a production-ready declaration.**
...
### 0.7 Crossagents rounds R1–R3 — 2026-09-22 UTC
...
**Closed in these rounds:** one Important trust defect (loopback locality gates admitted proxied remote clients: `8c926d9cb`), rate-limit keying behind trusted proxies (`e5f8b63ce`, `ff37f9845`), reset/hydration and sequence-space correctness (`23f4d4283`), the only source of managed-root HTTP 500s (`65ef2bcdd`, `358941c23`), three qualification-harness defects that had made every trusted-input cell unmeasurable (`4db06846c`, `79e7f2e9d`, `672cabcff`), partial evidence on abort plus an early operation-map lock (`2ac4f2712`), the §0.6 event-amplification measurement (`a8fa9db3b`), and the remaining §0.6 connection regression tests (`8e8669bcb`). Security review of pairing, tokens, scopes, redaction, static serving and relay found no further defect.
...
| Gate                                 | Blocker                                                                                                                                                                              | Needed                                                                                                            |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| Capacity claim for §4 budgets        | Shared workstation never produced a capacity-eligible window (`capacityClaimAllowed: false`); `v2-perf-qualification.yml` cannot be dispatched until it exists on the default branch | Quiet dedicated Mac, or the hosted sustained run after merge to `master`                                          |
| Overnight soak                       | 26 h exceeds hosted limits                                                                                                                                                           | Enrolled self-hosted soak runner                                                                                  |
| Windows 11 + WSL2                    | NAT leg needs repository variable `WSL_ROOTFS_SHA256`; mirrored leg needs a self-hosted Windows 11 runner                                                                            | Maintainer sets the variable; runner enrollment                                                                   |
| Physical iPhone / lower-tier Android | No devices or runners                                                                                                                                                                | Devices plus runner labels                                                                                        |
| Real-provider strict journeys        | Claude/Codex runnable locally; Gemini has no credentials                                                                                                                             | Gemini credentials or an explicit decision to exclude it                                                          |
| Published N−1 persisted upgrade      | v1.8.0 ships no server artifact and predates D4; the first valid N−1 is the first v2 stable release                                                                                  | Product decision: accept `NO_PUBLISHED_SERVER_N1` for the first v2 release and require the gate from the next one |
| Windows standalone target            | No artifact exists                                                                                                                                                                   | Product decision on the advertised target matrix                                                                  |
