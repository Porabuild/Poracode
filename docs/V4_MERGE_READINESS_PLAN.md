...
| F12 | **Final UI buffers and presentation costs remain.** Xterm hydration now caps its parser-side live suffix and resynchronizes from authoritative scrollback, but live writes still have no app-level queue accounting. Text smoothing adds a 240 ms drain target, while outer markdown normalization rescans revealed prefixes (`useSmoothStreamedText.ts:3`; `ItemMarkdownInner.ts:174`).                      | Bound the remaining final-consumer queues and keep visible work within the frame budget.                                       |
