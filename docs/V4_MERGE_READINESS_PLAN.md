...
| F7  | **Per-window delivery ownership is implemented at the backend boundary through two review rounds; live multi-window qualification remains open.** Main pushes each window's minted grant+interests plus the shell-remainder role; the backend splits bulk from controls, crosses bulk only as targeted sequenced copies to unowned windows (with generation-fenced recovery barriers on socket loss, failing open to full rebuilds when loss hints cannot prove coverage), keeps the shell-recipient window's controls exact-once, scopes terminal-bootstrap retention to the authenticated request origin, and rejects stale-generation targeted data (host 13 / stream 5 / facade 10; the former grant-less sentinel was removed after review confirmed no supported producer). Composed regressions cover disjoint/shared windows, mixed batches, close-skew, backpressure, stale generations, loss-map truncation, and bootstrap attribution, but the real Electron multi-window acceptance run has not run. | Prove steady-state zero bulk through main and socket-loss recovery in the live two-window acceptance scenario.                 |
...
Phase 3 items 1-3 are now implemented in code (uncommitted; see the execution
log): per-window grants/interests and acknowledged handoff cursors, targeted
fallback copies with generation-fenced recovery barriers, and mixed-batch
splitting at backend-host 13 / renderer stream 5 / facade 10, with composed
regressions on the production transport/relay paths. Items 4-5 remain open, and
the Phase 3 acceptance run (multi-window live qualification, zero steady-state
bulk bytes through main) has not been executed — the milestone stays open until
