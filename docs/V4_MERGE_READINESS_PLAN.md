...
6. Budget replies as well as events. `BackendRendererStream.sendReply` now uses
   ...
   and measure total bytes in flight across the handoff.
...
stream 5 / facade 10, with composed regressions on the production transport/
relay paths (including a real forked backend-IPC relay/dispatcher regression).
...
qualification. Items 4-5 and the wider Phase 3 acceptance (eight producers,
large history/attachment traffic, continuous typing/resizing/native-menu use)
remain open; the milestone stays open until that evidence exists.
