...
| 2026-09-18 | 2     | 2.5            | `200e2c7c4`                               | PARTIAL by escalation rule: the redundant renderer-stream leg is fully deleted (~9,300 lines; grants, targeted copies, chunk framing, version constants) — one event path + one request path with 32 MiB IPC byte-exact proofs and 8,786 tests green. The loopback unification sub-item STOPPED with wire-level evidence (PTY bytes off the replayable stream; remotely-consumed event set withholds desktop families) documented in versioning.md. G4 matrix before/after not re-run (baseline predated the boot fixes + build failure); opt-in suites within noise. |
| 2026-09-18 | 5     | 5.2            | `b88bd2c91`                               | Pairing state machine generated from one spec: TS reference executor (39 mirrored vectors) + Swift/Kotlin generated machines replacing the hand-written ones; native-bindings manifest 1→2 with all mirrored pins moved; wire IR byte-identical; parity tapes green; iOS AppTests + Android JVM suites pass. Terminal-cursor SM is the next slice.                                                                                                                                                                                                                    |
| 2026-09-18 | —     | hotfixes       | `1841aa84b`, `d6b1a7452`, _(this commit)_ | Probe-surfaced startup defects: no-database profiles classify fresh (userData is non-custodial by construction); startup dialog auto-quiets under PORACODE_CDP_PORT/CI/PORACODE_STARTUP_FAILURE_MODE=quiet (exit 1); Chromium singleton symlinks skipped (not refused) by promotion staging; electron kept out of the standalone server bundle (production build restored).                                                                                                                                                                                           |
