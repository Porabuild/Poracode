...
## 1g. Gates 2–3 Batch 2 — credential custody + standalone installable (2026-09-16, committed 0ad8403f3..e0a65492f)
...
Keychain incident (user-facing, resolved at machine level; see also the split plan below): recurring blocking "Keychain Not Found — store 'Lightcode Key'" dialogs were traced with live attribution to freshly-spawned Electron child processes failing per-process macOS SecKeychain resolution (Tahoe 26.x regression class; the user's keychain itself verified healthy). A stale 44h Nightly instance also squatted the stable app's shared Electron singleton, silently blocking stable launches. Remediation: app restart (reduced frequency), a title-matched auto-cancel watcher for residual occurrences, and a standing reboot recommendation (user's decision). NOTE: the earlier `tmp/` tree wipe was **the user's own planned disk cleanup** (their disk-space message listed `rm -rf ~/work/Poracode/tmp ~/work/Poracode/.tmp`; host evidence: 3.5 GB → 234 GB free, XCTestDevices 263 clones → 0) — `tmp/` is not durable storage; this record lives here for that reason. Bulky evidence logs stay under `.tmp/` on a vanish-any-time basis.
...
**Standing session note:** a title-matched SecurityAgent watcher ("Keychain Not Found" only — never touches other prompts) may be active on this machine to auto-cancel residual dialogs; tear it down when the session ends.

### Gate-4 regenerated matrix (2026-09-16, quiet host, v2 @ 0ad8403f3 vs master @ 0c99e7e136)

Merge-critical item 2 delivered: 12/12 ABAB sessions from clean detached worktrees, identical production-build recipe, both arms under the same `--use-mock-keychain` condition (0 dialog episodes post-switch; 3 pre-switch episodes preserved as excluded reps). Full evidence mirrored to `.tmp/v4-g4-batch2-mirror/` (5.2 GB — survives a tmp/ wipe); report `tmp/v4-g4-batch2/SESSIONS.md` (pre-wipe sections marked SUPERSEDED). Verdicts: payload budgets (shell60, hist40, tail independence) **PASS byte-identical**; cold-readiness **confound resolved — WITHIN-ALLOWANCE** (v2 +245 ms p50 vs master, under the 500 ms floor; the earlier FLAG was external-load contamination); tree memory/CPU **WITHIN-ALLOWANCE, v2 lower** (3.66 vs 5.33 GB peak). Standing items: deficit C-G4 #1 (oversized item inside the visible tail pulled whole — 8.4 MB, byte-identical on both arms) remains inherited-from-master and a recorded non-blocker; deficit C-G4 #2 narrowed — desktop-main seconds-scale startup stalls are **gone** (worst window 4378.9 → 68.4 ms; steady windows 26–31 ms against the 25 ms p99 budget, marginally over) while supervisor remains over (p99 112.4 ms); scaling hazard #3 stands (full host thread-list transfer 380 KB @ 1069 threads, every rep). Both remaining #2 shapes and hazard #3 feed Gate-4 Batch 3 (off-thread client engine extraction). Measured at the Batch-2 milestone SHA; intervening commits through the current HEAD touch tests/docs/gitignore only — no measured-surface runtime changes.
