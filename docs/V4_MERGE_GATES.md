...
**Final full `test`: GREEN — 14,019 passed / 119 skipped / 0 failed (1,254 files + 5 skipped)** on the complete tree at the follow-up fix SHA `5055c4ca5` (milestone + artifact repairs + opencode symlink fix; log `tmp/.full-suite-5055.log`). The two "race" UI tests (`PortsPanel`, `PrWatchControls`) turned out already corrected in the milestone's versions of those files — verified directly 25/25 — so the prepared stale patch was not needed; a first full-suite run also exited 0 before the count-capturing re-run.

**Publication:** milestone commit `120fcae09` pushed non-force to `origin/poracode/v2` (master ancestor `0c99e7e136` rechecked unchanged). CI took three cycles to green, each failure a missed generated-artifact regeneration in the focused correction cycles — recorded honestly: (1) `120fcae09`: push+PR core CI failed shards 6/8+7/8 on the two generated-contract drift guards (the checkpoint schema change required regenerating the remote v3 artifacts); repaired at `08d8f1724` (regenerated IR/schema/inventory/native bindings, new authoritative sourceHash `b137c859…`, `protocolVersion` stays 12, counts unchanged; 13/13 protocol tests + generator `--check` + typecheck green). (2) `08d8f1724`: core CI green both push+PR; both Native runs failed the native-e2e operation-map inventory lock (a second, separate committed manifest pin still on the old hash); repaired at `7c99fd844` with the full native e2e suite re-run locally (178 pass / 1 skip), not just the drifted file. (3) `7c99fd844`: **all four runs green — push CI `35026079123`, PR CI `35026085518`, push Native `35026079126`, PR Native `35026085594`** (Android 17 emulator job passed). Exact-published-SHA CI green achieved for the milestone chain; the protocol-regeneration step is now a documented boundary lesson for any future contract-file change.

- Publication recheck (root HEAD, `origin/poracode/v2`, master ancestor, candidate CI SHA): done — HEAD `7c99fd844` == origin, master ancestor `0c99e7e136` unchanged, CI green at that SHA.
- Follow-up fix SHA `5055c4ca5` (opencode symlink directory canonicalization, §1e BigPickle item): CI green after one rerun — push CI `35043964554` ✓, PR CI `35043969828` ✓, push Native `35043964550` ✓ (Android 17 + iOS jobs included), PR Native `35043969790` first attempt failed a single timing-sensitive test (`AppSessionCompositionTests.testBackgroundDuringAfterCommitReconcilesOnForeground`) that passed in the push run at the identical SHA — flake-class; rerun green with zero failed jobs. The flake is recorded as a known-flaky follow-up (its file already carries the milestone's timeout diagnostics; a repeat failure gets a real diagnosis from the evidence bundle).
...
## 1f. Gates 2–3 Batch 1 — "one authority" (2026-09-16, committed a5f34feca)

Batch-boundary verified per the batching rules: three authoring lanes with declared disjoint ownership (1A desktop shell/lease, 1B settings authority, 1C integrations/evidence/audit), one consolidated independent critic (verdict FIX-FIRST — all findings corrected before commit), broad checks all green (typecheck, lint both modes, fmt, protocol no-op as predicted, builds, full vitest **14,053 passed / 0 failed**, native:e2e **184 passed / 1 env-skip** incl. the previously-regressed sharedHostConcurrency now green), live drills on the packaged artifact (S2.3 stop drill GREEN; S3.4 concurrent-edit drill GREEN; zero-client continuity GREEN; two-real-Electron: ownership kind=desktop + control publication + loud admission refusal with correct guidance + no second DB writer + surviving first instance — refusal screen awaits dismissal, recorded as follow-up).

Landed: desktop owners publish discovery (describe-only control server, join-before-remove); `hostDataFence` custody fence handed to the backend child via the real initialize payload (critic-caught dead wiring fixed with a forked-child regression test); legacy pre-lease owner refusal; bounded wait-and-reprobe; attach generation re-verification on renderer (re)bootstrap (per-request pinning deliberately absent — preserves bearer-restart continuity, documented); zero-window command queue + OS-notification fallback; **the settings authority is live in both compositions** (was fully built with zero consumers) — every writer commits as revision-checked scoped edits, remote conflicts explicit 409/429, tailscale revert a value-guarded CAS, delta-map fields round-trip as written (root-cause fix for default-key unioning), facade runtime 11→12 with version-11 rejection regression; headless PR-merge durable effect; dead `threadSyncBroadcast` removed. Audit artifact: 319 channels, 273 renderer-independent (tmp/v4-g23-batch1/lane-1c/main-handler-audit.md).

CI at `a5f34feca`: push CI, PR CI, PR Native green first-try. Push Native failed the Android 17 job twice — **diagnosed pre-existing flake, not a regression**: the job runs the mock-mode lab (Batch-1 server code never executes there), zero android/ changes, the same SHA passed the same job in the PR run, the real-host job that does exercise the batch was green in the failing run, and the same journey failed pre-batch with the same push-fail/PR-pass signature. Mechanism (from the attempt-1 evidence artifact): the refresh-transcript click is swallowed by the disabled-during-interrupt-settling control plus a GC jank spike — a lost click cannot be waited out. Test-only hardening landed (quiescence gate + assertIsEnabled at that call site, matching the file's existing send-button pattern). This job remains the month-scale intermittent tracked item; a recurrence with the hardened test gets the journal-upload follow-up.

Deferred by decision (recorded): desktop-owner attach admission stays `"refuse"` until its dedicated live attach drill exists (Batch 2); renderer-socket-level re-verify and attach window-bounds remainders per the audit.

## 1g. Gates 2–3 Batch 2 — credential custody + standalone installable (2026-09-16, committed 0ad8403f3..cda9ca4ff)

Batch-boundary verified per the batching rules: two authoring lanes (2A custody, 2B packaging) with disjoint ownership, one consolidated independent critic (FIX-FIRST: I-1 crash-window test + I-2 versioning.md rows — both landed; verdict "no Critical findings"), broad checks 9/9 effective green (full vitest **14,145 / 0** twice; native:e2e 184/1; one unhandled-error class root-caused and hardened — usageRecorder flush now honors its fire-and-forget contract against torn-down bridges), **live custody drill on a real machine, all legs**: staging via the production `stageHostImport` (sha256-anchored receipt), clean typed refusal without desktop cooperation, one-time loopback key adoption between a live desktop and the `activate` CLI (offer retired on answer; adopted-key fingerprint byte-verified against the decoded headless key), manifest flip + record + archive, headless restart custody, SIGTERM drain, verified backup; **out-of-checkout prefix qualification 7/7** (assembled tarball sha256 583dce8d…, layout inference, loud `ServerLayoutError`, dependency confinement inside the prefix). Boundary discovery fixed en route: the desktop never mounted `HostCredentialAdoptionService` — mounted alongside the control server (same lease/lifetime) so activation is end-to-end functional; cooperation failures surface as the typed disclosure with cause detail. Qualification doc fixes landed in the same commit (install-script paren bug, declaration-env caution, ownership-before-layout note, appVersion must-pin).

CI story: cycle 1 (`0ad8403f3`) failed shards 3/8 + 8/8 — two distinct harness dependencies on prepared checkout resources that CI test shards do not stage (declared asset dir; ambient resolution of the signal-test child's esbuild externals). Fixed in `463f09faf` (test-owned temp asset dir) and `3032d16b0` (`NODE_PATH` names the repo's node_modules at fork; verified both ways in isolation). `995f8fc6d` added the harness mock-keychain opt-in, hardened same-day by `39ea0d888` (packaged launches never downgrade via env — `app.isPackaged` is authoritative over `isDev` and `PORACODE_USE_MOCK_KEYCHAIN`). Cycle at the final SHA is the milestone's CI verdict; push supercedes intermediate cycles via the concurrency group.

Keychain incident (user-facing; machine-level cause OPEN — see the split plan below): recurring blocking "Keychain Not Found — store 'Lightcode Key'" dialogs were traced with live attribution to freshly-spawned Electron child processes failing per-process macOS SecKeychain resolution (Tahoe 26.x regression class; the user's keychain itself verified healthy). A stale 44h Nightly instance also squatted the stable app's shared Electron singleton, silently blocking stable launches. **A reboot was tried 2026-09-16 14:24:51** (`kern.boottime`; the 14:27 Nightly start is a fresh-boot launch, not a quit/relaunch) and post-reboot dialogs still occurred. Attribution refinement from later evidence: every post-reboot episode coincides timewise with the unmocked Gate-4 measurement sessions (stable-identity production Electrons, 15:0x–16:0x — earlier "Nightly renderer" attribution predated knowing those sessions were running and the snapshot filter excluded them); since those sessions went `--use-mock-keychain` and finished, a continuously-armed title-matched watcher has observed **zero** dialogs; a deliberate real-keychain dev boot running the full app-side credential path (probe + decrypt) also produced **no dialog**. So the machine state is either resolved-by-reboot or still-latent-intermittent — unproven either way, and the product is treated as running in that environment regardless (no reboot recommendation stands; it was tried). NOTE: the earlier `tmp/` tree wipe was **the user's own planned disk cleanup** (their disk-space message listed `rm -rf ~/work/Poracode/tmp ~/work/Poracode/.tmp`; host evidence: 3.5 GB → 234 GB free, XCTestDevices 263 clones → 0) — `tmp/` is not durable storage; this record lives here for that reason. Bulky evidence logs stay under `.tmp/` on a vanish-any-time basis.

**P3 re-evaluation (coordinator directive):** the app-side per-operation probing pattern is statically confirmed — `src/main/backend/nativeCredentialCodec.ts:10` calls `safeStorage.isEncryptionAvailable()` on every seal/unseal request before `encryptString`/`decryptString` (:66-67); `src/main/secretStorageKey.ts:82/:112` and the adoption unseal hook (`src/main/main.ts`, `HostCredentialAdoptionService` mount) do the same. If the machine's failing state recurs, per-operation probing multiplies dialogs (macOS lookup failures are not cached by Chromium). Dynamic confirmation was NOT achieved: the packaged Nightly keeps no persistent main-process log to correlate against, and the deliberate trigger (fresh dev boot on the real keychain) ran the full path with zero dialogs in the current machine state. P3 design stands: probe safeStorage once per launch, latch keychain-unavailable into session-only mode with a user-visible notice, no in-session re-probe except on explicit user action — bounds app-originated dialogs to at most one per launch; it cannot stop Chromium-internal cookie-encryption dialogs raised by the network service. **PROPOSED for user decision: promote P3 to merge-critical** (small, confined to src/main credential modules, direct daily-user impact; include a persistent log line on the latch so future episodes leave evidence).

### Plan split (coordinator review, 2026-09-16)

**Merge-critical (before the v2→master merge):**

1. CI green at the release-candidate SHA — all four exact-SHA runs; **hold all pushes once the RC SHA's runs start** (the 995f8fc6d/e0a65492f pushes cancelled cycle 3 via the concurrency group; harmless then, not harmless at RC).
2. Gate 4 quiet-host re-run + verdicts at the final SHA (deficit C-G4 #2 — host-loop p99 during first-launch startup windows — needs the quiet-host measurement; C-G4 #1, the oversized-item-in-visible-tail pull, is inherited from master and recorded as a known non-blocker).
3. Gates 2–3 Batch 3 (operation integrity: mutation coordination, journal/receipts, shutdown, durability) — includes the dedicated desktop-attach drill and the admission-flip decision.
4. Gate 4 Batch 3 (off-thread client engine extraction).
5. P2 — never-silent single-instance refusal (read the SingletonLock holder PID, surface via log + OS notification instead of silent exit-0; the exact bug that made the user's stable app look dead).
6. Simulator/emulator reuse (user request, previously dropped): `scripts/native-e2e.mjs` `ios-ui` mode creates a per-run simulator (`simctl create "Poracode Native E2E <pid>"`, ~lines 234–240) with cleanup only in a finally block — the ios-ui branch exits (~line 60) before the signal handlers are installed (~line 164), so any killed run leaks a simulator and a per-run `.xcresult` dir (263 cloned simulators accumulated before the user's cleanup). Fix: reuse one fixed device (select via `selectIosSimulator` in `scripts/native-dev-lib.mjs` or a fixed name like the Android `poracode-pixel9-api37` convention), install shutdown handlers for that mode, cap/prune `.tmp/native-e2e/ios-ui-*` result dirs, add `xcrun simctl delete unavailable` to teardown. Android already uses a fixed AVD (no change). Manual `xcodebuild` commands in `docs/MOBILE_DEV.md` / `docs/RELEASE_MOBILE.md` use name-based destinations with no `-derivedDataPath` — switch to a fixed device id or a repo-managed derivedDataPath.
7. Soak (Gate 5: 24h soak + 72h observation).

**Post-merge:** P1 per-channel userData (versioned boundary + migration — pull forward only if the migration design proves short), P3 keychain-unavailable degradation split (cannot prevent the OS modal; product-side polish only), P4 shipped-build os_crypt identity investigation (low priority).

**Standing session note:** a title-matched SecurityAgent watcher ("Keychain Not Found" only — never touches other prompts) was NOT active as of 16:10; re-armed 16:4x (pid 64647). If re-armed again, record its pid here; tear it down when the session ends.

### Gate-4 regenerated matrix (2026-09-16, quiet host, v2 @ 0ad8403f3 vs master @ 0c99e7e136)
...
## 1i. Batch 3 — operation integrity, bounded surfaces, remote fairness, small fixes (2026-09-16)

Five lanes (1 operation-integrity+attach, 2 bounded thread-list surfaces + off-thread proposal, 3 remote-path fairness + origin isolation, 5 small-fixes bundle; lanes 3+4 merged for file ownership), one consolidated critic (FIX-FIRST; all items landed incl. the security pass with no Critical findings and four ratified deviations), broad battery 9/9 effective (full suite 14,213/1-out-of-freeze-flake; native 184/1), and the boundary evidence recorded durably in `docs/evidence/2026-09-16-batch3-boundary.md` (the tmp/ tree was externally wiped a second time during this boundary, taking the Batch-2 G4 raw evidence and the shared harness with it — the durability rule proved necessary the day it was written).

Landed: **operation journal** (`hostOperationJournal` v1: bounded, lease-checked, fingerprint-only) with activation crash-resume (resume/supersede/typed `HostActivationInterruptedError`); **delete-mid-revert guard** (RPC deletes refuse on running reverts with the retry remedy; between-phase revalidation stops file-restore into deleted/moved projects; the sync mirror settles doomed rows to `ambiguous` inside its transaction — both sync paths); **bare-replace loud-reject** at the RPC boundary (ratified: zero RPC callers, no replace-shaped event exists to fabricate); **taskkill bound** (5 s timeout + fallback, the single Windows tree-kill choke point); **bounded thread-list surfaces** (dbGetThreadsPage keyset pagination, `/api/snapshot?threadLimit` + cursor `GET /api/threads`, facade 12→13 with rejection regression, client ticket-migration for `<img>`, all artifacts regenerated: 81 routes, operation-map 224); **remote-path reserved control capacity** (control-class admission ahead of bulk under saturation, fail-closed classification, isolation test over real HTTP); **origin isolation proven** (per-host child origins verified end-to-end; automated probe + suite green; manual Safari leg scripted with DNS/TLS prerequisites — user-environment residual); **P2** never-silent single-instance refusal (holder-PID disclosure + modal error box; parse hardened for AWS-shape hostnames); **P3** once-per-launch safeStorage latch (session-only + persistent log line; S5.2 rotation-refusal semantics preserved); **B5a** pairing-token redaction; **B5b** one-time image tickets (route contracted + ledgered; legacy query param deprecated pending full client migration — runtime-image route follow-up recorded); **production PWA qualification gate** (mirrors the nightly pattern; dispatch-only, exact-SHA); **simulator reuse** (fixed `Poracode Native E2E` device, signal handlers, result pruning, `simctl delete unavailable`; live-verified 1→2 devices with the delta being exactly the deliberate reuse device).

Live boundary (coordinator): **attach drill ALL 5 LEGS PASSED** — existing desktop profile migrates into the standalone authority and a desktop launch attaches (owner pid unchanged); the desktop-owner pairing refusal held live, validating `DESKTOP_OWNER_ATTACH_ADMISSION = "refuse"` (endpoint-null + rejected pairing make a flip strictly worse disclosure). **Gate-4 Batch-3 acceptance PASS**: supervisor 1.5 ms / desktop-main 1.2 ms / backend 1.4 ms pooled steady p99 against the 25 ms budget (measured on a byte-verified copy of the uncommitted tree, 6 sessions, zero dialog episodes); hazard-#3 wire bound ≤ 47 853 B per page at the 1069-thread fixture (27% headroom). Deficit C-G4 #2 and hazard #3 are **closed**; the off-thread client-engine seam remains a recorded proposal (lane 2's `tmp/v4-g4-lane2/PROPOSAL.md` summary: renderer Web Worker engine, staged) — not required by any current budget, next considered if future measurements regress.

**Defect found by the soak (open, next focused fix; characterization corrected by disk monitoring):** the supervisor's ACP adapter installer retry-loops a **full install cycle that always fails after extraction** — each ~30–45 min cycle downloads the complete 316,014,828 B `antigravity-acp` v1.1.1 archive (that is its full size, not a truncation), extracts ~1.06 GB into `1.1.1/bin/` (`agy_acp_server.par` 947 MB + helpers), then the install step fails and **cleanses** (`bin/` wiped, zip truncated) before retrying with no bound or backoff. Projected 24 h churn: 25–60 GB disk writes + 6–12 GB network; the failure never surfaces to any log the soak captures (the exact throw site is inside the post-extract install path — next session reproduces it against the live registry on a scratch baseDir and fixes at the source). Soak budgets unaffected (memory/integrity/latency clean; the wipe-before-retry keeps the root bounded). Fix shape unchanged: bounded retries with exponential backoff + a surfaced persistent failure state. Evidence: `tmp/soak-run/logs/acp-download.ndjson` + `acp-disk.ndjson` (both loggers detached; SOAK-PLAN.md "Observed events" carries the cycle timeline). Soak continues to its 24 h verdict — its memory/queue/latency/integrity surfaces are orthogonal to this churn.
...
## 1k. Android-17 emulator-runtime flake lane — signature table and fixes (2026-09-18, evidence through 423789b9a)

Every completed `Native clients` run on poracode/v2 (2026-09-12 → 2026-09-18; the workflow does not run on master) plus failed-run attempt logs and uploaded evidence artifacts were inspected (`gh` jobs/logs/artifacts of runs 35311903502, 35189930195, 35162770416, 35077890850, 34905657228; one 09-12 run for the pre-window era). The `Android 17 emulator runtime` job failed in 5 runs / 6 attempts, **all on `push` events**; every same-SHA `pull_request` run that reached completion was green (889d2d79e, a5f34feca, ffe4f0f89 → PR green while push failed; f359ff471/423789b9a PR runs were concurrency-cancelled before the job ran). Per-attempt signature table:

| Date (UTC)  | Run / SHA (event)                                           | Attempt | Terminal signature                                                                                                                                           | Classification                                                                                            | Disposition                                                                                    |
| ----------- | ----------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 09-14 22:45 | 34905657228 / ffe4f0f89 (push)                              | 1       | `ComposeTimeoutException: Condition still not satisfied after 2000 ms` at the stop-control click (test line 236 of the then-current file)                    | test-side: bare 2 s issuance bound after a click that the disabled-during-interrupt control could swallow | **Fixed earlier** by 043f8b7e (2026-09-16): `assertIsEnabled` quiescence gate before the click |
| 09-16 09:10 | 35077890850 / a5f34feca (push)                              | 1       | `AwaitTimeoutException: route:thread-history minimum=4 actual=3` at the refresh-click confirmation count (WireLabControl.kt:132)                             | test-side: same swallowed-click family                                                                    | **Fixed earlier** by 043f8b7e (quiescence gate at test lines 288–301)                          |
| 09-16 09:10 | 35077890850 / a5f34feca (push)                              | 2       | `am start -W` → `Status: timeout`, `LaunchState: UNKNOWN (-1)`, WaitTime 11247                                                                               | app-side cold-start slowness on the emulator colliding with the single-shot launcher wait                 | **Fixed here**: launch verification + retry in `scripts/ci-android-api37.sh`                   |
| 09-16 23:32 | 35162770416 / 889d2d79e (push)                              | 1       | `IllegalStateException: No compose hierarchies found in the app` at the first post-deny poll (`waitForText` → test line 112)                                 | test-side zero-root throw exposed by emulator slowness (see below)                                        | **Fixed here**: zero-root-tolerant polling in both instrumented tests                          |
| 09-17 06:27 | 35189930195 / f359ff471 (push)                              | 1       | `am start -W` → `Status: timeout`, WaitTime 10847                                                                                                            | same as a5f34feca attempt 2                                                                               | **Fixed here**: same script hardening                                                          |
| 09-18 05:54 | 35311903502 / 423789b9a (push, attempt 2 of the run passed) | 1       | `am start -W` returned `Status: ok` but `TotalTime: 11409` for a COLD launch; the journey then died with `No compose hierarchies` at the same post-deny poll | same two root causes in one run                                                                           | **Fixed here**: both fixes                                                                     |

Pre-window era (2026-09-12, 8 failed runs): `No device found matching --device pixel_9` AVD-profile failures — already fixed at HEAD by the "Pre-create the API 37 AVD" workflow step (comment in `.github/workflows/native-ci.yml`).

**Decisive artifact evidence (uploaded `android-api37-runtime-1` artifacts):**

- Measured cold launches of `MainActivity` on the 2-core software-rendered API 37 emulator: **6537 ms, 11409 ms** (`am start -W TotalTime`), with `ProfileInstaller` still installing the baseline profile _during_ the journey test and a `Choreographer: Skipped 50 frames` under load. Launch is genuinely slow; nothing in the logs convicts a specific main-thread sin (`MainActivity.onCreate` calls `setContent` before `session.bootstrap()`), so per the evidence rule the fix hardens the launcher wait instead of touching app code.
- The `Status: timeout` runs show `LaunchState: UNKNOWN (-1)` at ~10.8–11.2 s: the `am start -W` completion handshake starves out while the activity comes up moments later. The old script treated the single `Status:` line as ground truth with no retry.
- Journey-test per-test logcat (423789b9a): the UiAutomator deny click landed on the system `ACCESS_LOCAL_NETWORK` dialog **after its input channel was already gone** (`InputDispatcher: Not sending touch gesture … NO_INPUT_CHANNEL`), the launcher reached the foreground ~2 s later, and `VRI[MainActivity]` went invisible at the failure timestamp. During that window the app transiently has **zero registered Compose windows**, and `fetchSemanticsNodes` throws `IllegalStateException: No compose hierarchies found in the app` on the first poll instead of reporting empty — so `compose.waitUntil` aborted instantly even with a 30 s budget. The identical deny flow in `Android37MultihostInstrumentedTest.accessLocalNetworkDenyAndRevokeStayOnSemanticDeniedUi` (which runs unwaited `assertIsDisplayed` after `deny.click()`) passed in the same run — the race is low-probability timing, not app logic.
- Transient noise, **never terminal**: `adb: device offline` exit-1 on the first 1–2 `getprop sys.boot_completed` polls appears in every failing log and self-recovers inside the emulator-runner poll loop; boots complete in 53–54 s against the 600 s budget; `-no-snapshot` is already set, so no stale-snapshot boots; `bad color buffer handle` lines are software-GPU cosmetics. No boot-step retry/wipe was added — the evidence shows no terminal boot failure to fix.

**Fixes landed (this section, uncommitted):**

1. `scripts/ci-android-api37.sh` — (a) after `sys.boot_completed`, bounded ping of `pm path android` + `am get-current-user` before install so install/start can't race framework-services readiness; (b) the launch is verified by `pidof` + the activity-manager resumed-activity record (up to 60 s), falling back to one force-stop + relaunch, with the `am start -W` output kept as evidence — `Status: timeout` with the activity verifiably up no longer fails the job.
2. `android/app/src/androidTest/kotlin/com/poracode/app/Android37WireLabJourneyInstrumentedTest.kt` — `waitForText` polls through a zero-root-tolerant helper (treats the "No compose hierarchies" throw as "not yet", letting the 30 s budget absorb the post-dialog window), following the existing `assertIsEnabled` quiescence pattern from 043f8b7e.
3. `android/app/src/androidTest/kotlin/com/poracode/app/Android37MultihostInstrumentedTest.kt` — the post-deny `assertIsDisplayed` is now preceded by the same zero-root-tolerant wait (same signature, same job).
...
| Gate 2 — standalone ownership             | **DONE**                      | Batches 1–3 (§1f–§1i): one-authority lease, attach, settings authority, credential custody, live attach drill 5/5. Desktop-managed and standalone hosts share one owned `.host-v1` data root; automatic journaled promotion (V5 1.3 / V6 C.4) copies the plain profile on first managed launch.                                                                                                                                                                                                       |
...
| Gate 4 — off-thread client engine         | **DONE (smaller extraction)** | `src/renderer/state/remote/engine/`: decode/validate of backend-stream and remote-socket frames plus ≥32 KiB persist JSON run in a module Worker when available; same-tick sync fallback in tests. Bounded 256-frame queue, generation fencing, ACK of completed work, overflow → 1013 close / reconnect. Not the comprehensive worker-owned history/IndexedDB redesign (Phase 4 items 4.6–4.10 remain post-measurement). RuntimeEventQueue remains the UI drain.                                     |     |
...
| Contracts / versions                      | **DONE**                      | Generated-artifact provenance from `protocol/remote/v3/generated/inventory.json` + `tests/native-e2e/harness/operation-map.json` (do not type the counts here). Facade is `CLIENT_HOST_HOP_VERSION`. Old-reader rejections regression-covered.                                                                                                                                                                                                                                                        |
...
**Contract inventory update (2026-09-20, uncommitted B4 lane):** 84 routes, operation-map 274 — three additive bounded-read routes (`project-list`, `catalog-membership`, `thread-turns`); protocol stays 12 and the generated artifacts, native parity ledger and operation map are regenerated together.
...
Current position: consolidated review + broad checks + full live qualification completed 2026-09-15 on the final 66-path candidate (§1e): stream 17/17, standalone 8/8, desktop-web and iOS Safari journeys PASS, two product defects found and fixed in-candidate with three-surface live re-proof, BigPickle chat PASS via symlink-free control; the pre-existing opencode symlink defect found during qualification is now FIXED (`5055c4ca5`) with live before/after closure on both path shapes. Full suite GREEN twice on the complete tree — final count 14,019 passed / 119 skipped / 0 failed; the two "race" UI tests were already corrected in the milestone's file versions (25/25 direct). **Exact-published-SHA CI green at `7c99fd844` (all four runs, §1e publication record); CI rerunning at `5055c4ca5` with push CI already green.** Gate 1 close conditions all hold at `7c99fd844`; final confirmation records the fix-SHA CI when all four runs land.
...
§1d). **Current verdict: Gate 4 DONE on measured host-loop budget plus the smaller off-thread client-engine extraction — see §1l.**
