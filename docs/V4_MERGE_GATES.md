...
**Defect found by the soak (open, next focused fix):** the supervisor's ACP adapter installer retry-loops a download that can never complete on this network — the `antigravity-acp` v1.1.1 archive transfer breaks deterministically at byte 316,014,828 (~301.4 MiB; the downloader itself has no cap — `downloadToFile` is a clean stream pipeline, so the cut is network/CDN-side), and the install path (`acpRegistry.ts` → `binaryInstance` → `downloadToFile`) retries with no bound or backoff: ~29 min per attempt at ~180 KB/s, ≈10 GB/day of network+disk churn, adapter never installs, failure never surfaces. Soak budgets unaffected (RSS/integrity/latency clean; the partial file is overwritten in place). Evidence: `tmp/soak-run/logs/acp-download.ndjson` (5-min size samples, 3 full cycles). Fix shape: bounded retries with exponential backoff, a content-length precheck that refuses archives the registry itself sizes below the cut, and a surfaced persistent failure state instead of silent churn.
